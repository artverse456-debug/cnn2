...
export default function FanDashboard() {
  ...
  const handleJoinGroup = async (groupId: string) => {
    ...
    try {
      ...
      await supabase.from("group_memberships").insert({ user_id: session.user.id, group_id: groupId });
      setMessage("Du bist der Gruppe beigetreten.");
    } catch (error) {
      console.error("Join group failed", error);
      setMessage("Gruppenbeitritt fehlgeschlagen.");
    }
  };
  ...
  const handleClaimReward = async (rewardId: string) => {
    ...
    try {
      const supabase = getSupabaseBrowserClient();
      await supabase.from("reward_claims").insert({ user_id: session.user.id, reward_id: rewardId });
      setMessage("Reward eingelöst.");
    } catch (error) {
      console.error("Reward claim failed", error);
      setMessage("Reward konnte nicht eingelöst werden.");
    }
  };
        ...
        <DashboardCard title="Deine Gruppen" subtitle="community">
          {memberGroups.map((group) => (
            <div key={group.id} className="flex items-center justify-between rounded-2xl border border-white/10 px-4 py-3">
              ...
              <button className="text-sm text-primary-light" onClick={() => handleJoinGroup(group.id)}>
                Gruppe beitreten
              </button>
            </div>
          ))}
      ...
      <DashboardCard title="Eingelöste Rewards" subtitle="timeline">
        ...
        <div className="mt-4 flex justify-end">
          <button
            ...
            onClick={() => handleClaimReward("demo-reward")}
          >
            Reward einlösen
          </button>
        </div>
